    ...
    signIn = async () => {
        try {
            ...
            if(this.state.isSignedIn === true){
                ...
                header.method='GET';
                let url = "https://www.googleapis.com/gmail/v1/users/me/threads?labelIds=INBOX&q='from:@gmail.com'";
                header.headers["Authorization"]= 'Bearer '+ tokens.accessToken;
                fetch(url, header).then((response) => {
                    return response.json()
                })
                .then( async (responseJson) => {
                    console.log("This is the thread list");
                    console.log(responseJson.threads);
                    for(let i=0; i<responseJson.threads.length; i++){
                        const threadtable = firestore().collection(`${this.props.user.user.id}`).doc(`${responseJson.threads[i].id}`);
                        threadtable.set({
                            id: responseJson.threads[i].id,
                            snippet: responseJson.threads[i].snippet,
                            mute: false,
                            snoooze: false
                        });
                    }
...
export default connect(mapStateToProps, { createUser, savetoken, saveThreadList })(Login);